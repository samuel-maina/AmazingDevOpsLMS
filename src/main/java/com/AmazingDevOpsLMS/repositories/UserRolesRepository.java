...
public interface UserRolesRepository extends CrudRepository<UserRole, Long> {
    ...
    @Query("select u.user.id from UserRole u where u.roles.id=?1")
    public List<String>Users(String role_id);
    
    @Query("Select u.user from UserRole u where u.roles.id=?1")
    public List<User> findUsersByRole(String roleId);
    @MapKeyColumn(name="role")
    @Query("select u.roles.id as role, count(u.roles.id) as count from UserRole u group by u.roles.id")
